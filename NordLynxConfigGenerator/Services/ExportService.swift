...
nonisolated struct ExportService: Sendable {
    ...
    func exportAsIndividualFiles(configs: [GeneratedConfig], folderURL: URL) -> [URL] {
        configs.map { folderURL.appending(path: $0.fileName) }
    }

    func exportAsZip(configs: [GeneratedConfig], folderURL: URL, label: String) throws -> URL {
        try zipService.createZip(from: folderURL, zipName: label)
    }

    func exportAsMergedText(configs: [GeneratedConfig], label: String) throws -> URL {
    ...
    func exportAsJSON(configs: [GeneratedConfig], label: String) throws -> URL {
    ...
    func exportAsCSV(configs: [GeneratedConfig], label: String) throws -> URL {
