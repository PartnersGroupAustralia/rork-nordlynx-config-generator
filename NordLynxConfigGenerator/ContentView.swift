...
struct ContentView: View {
    ...
    @State private var selectedConfig: GeneratedConfig?
    ...
    private var protocolPicker: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 0) {
                ForEach(VPNProtocol.allCases) { proto in
                    Button {
    ...
    private func protocolAccent(for proto: VPNProtocol) -> Color {
        switch proto {
            ...
            LazyVStack(spacing: 10) {
                ForEach(Array(viewModel.filteredConfigs.enumerated()), id: \.element.id) { index, config in
                    Button {
                        ...
                        ConfigRow(config: config, accentColor: accentColor)
                    }
